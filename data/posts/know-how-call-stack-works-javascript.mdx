...
que invito a que revises después de terminar de ver esta post.

## Stack o Pila

Para continuar vamos a entender que es una pila o stack. Este es un algoritmo conocido por sus siglas
[LIFO](https://es.wikipedia.org/wiki/Last_in,_first_out) que significa **Last in, First Out**. Un ejemplo para entender
mejor el concepto es cuando pensamos en varios libros uno sobre otro, 📚 a esto lo llamamos una pila de libros,
si queremos poner un nuevo libro, lo hacemos poniéndolo encima de los que ya tenemos y si queremos obtener el libro del
medio, debemos ir quitando los últimos libros que pusimos que son los de arriba hasta llegar al que queremos.
...
por Javascript para saber en qué función se encuentra el punto de ejecución, poder agregar las siguientes funciones que
se van a ejecutar y por cuáles funciones ha pasado la ejecución antes de llegar a la actual. Así Javascript con esta
información podra saber a donde tiene que volver para seguir ejecutando la siguiente función.
...
<CallStack
  ...
  steps={[
    { stack: [0], line: 13, description: 'En la línea 13 comienza nuestra ejecución, esta función inicial es llamada como (anonymous), es la función principal de la ejecución, cuando esta función no exista es porque hemos terminado la ejecución.' },
...
esas variables a las cuales puedo acceder, es decir, que son "visibles" para una función.
...
* Ha entender como funciona Javascript y por ende al pensar en contruir nuestro codígo lo haremos mucho más rapido y
concientes.
...
de las herramientas del desarrollador del navegador se debe a que el call stack tiene un limite que depende del
navegador, puedes ver un poco más de este limite en el post [The maximum call stack size](https://2ality.com/2014/04/call-stack-size.html).
...
(anonymous)	@	VM4271:9
```
También tenemos las herramientas del desarrollador del navegador, por ejemplo en Chrome podemos acceder con la tecla
...
como agregar uno, y luego al lado derecho de nuestras herramientas de desarrollador podemos ver la sección con el nombre
`Call Stack` donde podemos ver nuestras funciones y como se va actualizando a medida que vamos avanzando en la ejecución.
